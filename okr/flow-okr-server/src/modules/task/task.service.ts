...
export class TaskService {
  constructor(
    ...
    @Inject('TASK_DETAIL_REPOSITORY')
    private taskDetailRepository: Repository<TaskDetail>,
  ) {}
  ...
  async addTaskDetail(dto: AddTaskDetailRequestDto) {
    const targetTask = await this.taskRepository.findOneBy({ id: dto.taskId });
    if (!targetTask) throw new Error('Target Task not found');

    const newTaskDetail = this.taskDetailRepository.create({
      task: targetTask,
    ...
    await this.taskDetailRepository.save(newTaskDetail);
  ...
  async getTotalTaskTree(): Promise<any> {
    ...
    function buildTree(nodeId: number): any {
      ...
      const children = closures
        .filter(
          (closure) => closure.ancestor.id === nodeId && closure.depth > 0,
        )
    ...
    const rootClosure = closures.find(
      (closure) =>
        closure.depth === 0 && closure.ancestor.id === closure.descendant.id,
    );
  ...
  async getTaskDetail(taskId: number) {
    ...
    if (task && task.detail) {
      ...
      console.log('TaskDetail not found for the given task.');
      return null;
    }
  }
}